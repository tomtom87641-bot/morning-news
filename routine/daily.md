...
4. 原稿を自己点検する：字数（Python）、算用数字・アルファベットが無いか（`grep -nP '[0-9A-Za-z]' "$HOME/work/script.txt"`）、日付・曜日が手順0と一致するか、出典名のない断定が無いか、**各分野の冒頭に分野・制度自体の定義文が入っていないか**（手順3の禁止例のような一文が無いか、原稿を読み返して確かめる）。見つかったら直す。
