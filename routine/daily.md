...
1. 次の6分野を、この順に、WebSearch で探す。各分野2〜3話題、直近24時間を優先（無ければ直近3日）。
   ...
   **⑤マダミス・謎解きと⑥東京のイベントは、名前を挙げるだけでは伝わらないので、検索結果から次のどれかが分かる範囲で拾う**：⑤ならその**作品固有**の世界観・テーマ（例：復讐劇、宮廷もの、と作品ごとに違う内容）、何人で遊ぶ形式か、あらすじの一言。⑥なら何をする催しか（例：屋台の並ぶ祭り、作品を展示する催し、音楽のイベント）。検索結果に書かれていない中身は想像で補わない。**「マーダーミステリーとは参加者が役を演じて謎を解く遊びです」のような、ジャンルそのものの定義説明は書かない**（下の手順3の禁止事項も参照）。
...
3. 読み上げ原稿を `$HOME/work/script.txt` に、Bash の `cat > "$HOME/work/script.txt" <<'EOF'` で書く。次の規則を守る。
   ...
   - **作品名・イベント名など固有名詞を出すときは、名前だけで終わらせない。** それが何なのか（内容・何をする催しか）を必ず一言添える。特にマダミス・謎解き・東京のイベントの分野は、名前と日付だけの紹介にしない。
